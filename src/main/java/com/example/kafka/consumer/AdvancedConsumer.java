...
public class AdvancedConsumer {
    ...
    public AdvancedConsumer() {
        ...
        int threadPoolSize = calculateOptimalThreadPoolSize(cpuCount);
        this.executorService = Executors.newFixedThreadPool(threadPoolSize);
        
        logger.info("系统CPU核心数: {}, 线程池大小: {}", cpuCount, threadPoolSize);
    }
    
    /**
     * 自定义线程池大小的构造函数
     */
    public AdvancedConsumer(int threadPoolSize) {
        Properties props = KafkaConfig.getConsumerConfig();
        this.consumer = new KafkaConsumer<>(props);
        
        // 验证线程池大小是否合理
        int cpuCount = Runtime.getRuntime().availableProcessors();
        int finalThreadPoolSize = Math.max(1, Math.min(32, threadPoolSize));
        
        this.executorService = Executors.newFixedThreadPool(finalThreadPoolSize);
        
        logger.info("系统CPU核心数: {}, 自定义线程池大小: {}", cpuCount, finalThreadPoolSize);
    }
    
    /**
     * 计算最优线程池大小
     */
    private int calculateOptimalThreadPoolSize(int cpuCount) {
        // 对于Kafka消费（混合型任务），推荐使用 CPU核心数 * 2
        // 但也要考虑最小值和最大值
        int optimalSize = cpuCount * 2;
        
        // 设置合理的范围
        int minThreads = 2;  // 最少2个线程
        int maxThreads = 16; // 最多16个线程，避免过多线程导致上下文切换开销
        
        return Math.max(minThreads, Math.min(maxThreads, optimalSize));
    }
    ...
    public void stop() {
        this.running = false;
        logger.info("高级消费者停止");
        // 注意：这里不需要中断线程，因为主循环会检查 running 状态
    }
